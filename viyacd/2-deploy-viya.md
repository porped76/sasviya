...
# Save the the PATH info for the next you login and check Kustomize
ansible localhost -m lineinfile -a "dest=~/.bashrc regexp='^export PATH' line='export PATH=$PATH:$HOME/clouddrive/payload/kustomize'" --diff
kustomize version
...
# Apply the k8s manifests and start the deployment. It will take ~30 minutes
# Apply the Cluster-Wide level objects
kubectl apply -n ${NS} -f site.yaml --selector="sas.com/admin=cluster-wide" --prune
## wait for these resources to exist
kubectl apply -n ${NS} wait --for condition=established --timeout=60s -l "sas.com/admin=cluster-wide" crd
# Apply the Cluster-Local level objects
kubectl apply -n ${NS} -f site.yaml --selector="sas.com/admin=cluster-local" --prune
# Apply for namespace-level objects
kubectl apply -n ${NS} -f site.yaml --selector="sas.com/admin=namespace" --prune

# Simple way to Monitor the Viya deployment
watch kubectl get pods -o wide -n $NS

# Advanced way to Monitor the Viya deployment. Force the $NS and the $KUBECONFIG variables
#SessName=deploy_watch
#tmux new -s $SessName -d
#tmux send-keys -t $SessName "NS=sasviya-prod && PERFIX=ibe && export KUBECONFIG=~/.kube/${PERFIX}-aks-kubeconfig.conf &&\
...
# Make sure that you are using the Cary Direct VPN (profile 3 usually) and then connect to the SAS Viya applications
