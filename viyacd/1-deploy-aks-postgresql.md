...
CLOUDBOXIP=$(curl https://ifconfig.me)
# Define k8s config
KUBECONFIG=~/.kube/${PERFIX}-aks-kubeconfig.conf
# Save the the KUBECONFIG info for the next you login
...
# Clone the Gitlab project and extract the payloads tarball (Open LDAP and sitedefault.yaml, license and useful deployment artifacts)
rm -Rf ~/clouddrive/payload && rm -Rf ~/clouddrive/ops && rm -Rf ~/clouddrive/project && rm -Rf ~/viya && mkdir ~/clouddrive
git clone https://gitlab.sas.com/porped/viya.git
cp ~/viya/viyacd/payload*.tgz ~/clouddrive/payload.tgz
cd ~/clouddrive
tar -xvf payload.tgz && rm -f payload.tgz
...
# Stayin on the Terraform 13.2 version, because of issues with v13.1 that comes by default in Azure Cloud Shell
TFVERS=0.13.2
...
# Deploy the AKS cluster with the Terraform plan
TFPLAN=my-aks.plan
cd ~/clouddrive/project/aks/azure-aks-4-viya-master
